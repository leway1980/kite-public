...
const aiEduStories: Story[] = [
	...
	},
	{
		...
		category: 'AI × 教育',
		title: '郵寄制學習能否提升技能進階？',
		short_summary: '這篇由 NBER 發表的工作論文聚焦 20 世紀初美國的通信學校（International Correspondence Schools, ICS），以新數位化學員記錄與人口普查資料做資料串聯，建立可追蹤的個體樣本。研究發現，參與 ICS 的學生在 3 到 10 年後，其取得熟練型就業機率高出 6 到 10 個百分點；尤其對年紀較輕、將通信課程視為高中替代的人更明顯。該設計採長期追蹤、量化估計與 Roy 式一般均衡模型，並非單次問卷調查，而是觀察學習軌道如何影響職涯分流。\n\n此研究的重要性在於，它把替代教育納入正式勞動市場結果，而不只視為補習工具。對教育科技與教育政策而言，含義是「替代路徑」本身可改變能力選擇順序：當某些學生先以對應課程累積技能，再面對升學或就業決策，可能提升整體匹配效率。也就是說，非正規學習設計不一定是「第二梯次」、而可能是提前進場的主軌之一。\n\n對台灣高中教師，實作重點不是搬用歷史郵寄制，而是把“任務型替代學習”變成可評核路徑：可先為低資源、學習步調慢的學生設置分階段作品任務與能力作品包，並以季度檢核代替單次考試；同時以 1 年、3 年追蹤畢業後升學／就業去驗證。落地上要顧及台灣的學分與升學規範，先在校內訂立可核可的替代證明機制，再向地方教研群做對齊，否則容易落入只做加值課程、卻無法轉成制度認列的窘境。',
		emoji: '📬',
		did_you_know: '公開摘要揭露 ICS 到 1940 前累積超過 4,000,000 名學生，且參與者在 3 到 10 年後的熟練工作機率提升 6 到 10 個百分點，顯示非傳統學制可改變勞動市場進入機制。注意：本研究為觀察性研究，不宜直接推論因果。',
		talking_points: [
			'證據類型：NBER 工作論文的觀察性研究，核心為歷史通信學校個體紀錄與人口普查資料鏈結，並搭配一般均衡 Roy-style 模型估計機制。',
			'數據佐證：公開摘要揭露 ICS 在 1940 年前累積超過 4,000,000 名學生；參與者在 3–10 年後熟練就業機率高出 6–10 百分點；摘要未揭露信賴區間、子群精細樣本數。',
			'研究設計：場域為美國早期，聚焦“以通信課程替代/補足高中”的教育軌道，透過長期追蹤（3–10 年）觀察技能就業結果，而非即時學科成績。',
			'核心機制：模型指出通信教育在低能力族群可形成早期技能補位，並改變學生在「高學歷」與「技能就業」之間的選擇排序。',
			'時間規劃：效應被定義在 3 年以上觀察窗，暗示政策驗證至少要有 1 年、3 年、10 年階段性指標，才可比較短期投入與長期就業回報。',
			'可遷移做法：台灣教師可借鏡「任務式替代」概念，用季度作品檢核與技能檔案取代部份傳統補修流程，先針對高風險學生做分組試行，再決定是否納入正式學分替代。',
			'風險提醒：研究未直接處理台灣升學考試與高職認證制度，未滿足在地法規條件前，不宜把替代路徑直接宣告成可複製的通行版本。',
		],
		...
		perspectives: [
			{
				text: '第一種解讀是替代學制可作為技能入口：研究顯示，將學習路徑多元化，不必全然等同學歷門檻，尤其能幫較年輕、難以即時完成高中者先進入技能職涯；但這是長期追蹤結果，不代表即時教學的短效解。',
				sources: [
					{
						name: 'NBER',
			...
			{
				text: '另一路向是制度張力：證據來自歷史美國、且無 RCT 設計，若直接移植到台灣，可能與升學考試與學分認證衝突。延伸研究缺口在於，需增加本地對照文獻與近代線上學習資料，才可建立可操作的政策推論。',
				sources: [
					{
						name: 'NBER',
						url: 'https://www.nber.org/papers/w35147#fromrss',
...
const twEduStories: Story[] = [
	...
	},
	{
		...
		number_of_titles: 3,
		category: '教育政策',
		title: '本週AI融入教學高中：三案例落地方式',
		short_summary: '2026-05-02 與 2026-05-06 共 3 則快訊形成「AI 融入教學 高中」叢集，來源為文匯報、荷花、中央社；去重後仍有 3 個獨立訊號、同質轉播 0 則。三個明確訊號是：元朗商會中學於 2024/25 學年推進 AI 資優跨域專題；大角嘴天主教小學把 AI 放入 VARK 四種學習類型教學流程；東華大學觀遊系將生成式AI導入「馬太鞍自由行」導覽教學。這些訊號共同揭示，AI 正在從概念演練走向可交付的課務流程。\n此趨勢意義在於，AI 不再只出現在資訊科技課，而是同時出現在語文、跨學科專題、導覽與社會情境學習；學校將 AI 定位為「學習路徑設計」與「自學回饋」的工具，而非單向輸出平台。這代表 AI 融入教學的核心不只是硬體導入，而是任務設計、評量機制與價值導向的再分配。目前可確認的主訊號是實作樣態，但仍缺少成本、人時、成效量化與規模化落地證據。\n對台灣高中教師的直接啟示是：先以可控規模做「AI 任務切片」，例如 1 次課堂做一個跨學科導覽腳本，配合學生學習類型分組，再用 AI 生成範例由學生批改修正。再建立每週追蹤表，紀錄 AI 回饋、學生修正與教學調整，避免變成只會貼上工具的作業。高二/高三可先對接歷史、地理、生活科技等科目，並鎖定 10-20 人試行；校內共備與行政支援到位後再擴至年級制，這比一次到位的全面上線更能降低失敗風險。',
		emoji: '🤖',
		...
		talking_points: [
			'本叢集來自 3 則原始快訊（2026-05-02 兩則、2026-05-06 一則），去重後 3 個來源訊號仍保留獨立觀點，明確可確認「同質轉播」為 0。',
		...
		perspectives: [
			{
				text: '文匯報訊號聚焦高中資優教育制度化經驗，重點在「學中用、用中學」與社會責任導向，呈現的是學習方法論的重構；荷花則偏向基礎學校治理，強調日課配置、作息制度與學習差異化，兩者都談工具但切入問題完全不同。',
				sources: [
					{
						name: '香港文匯網 - 文匯報',
