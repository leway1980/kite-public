...
const educationResearchStories: Story[] = [
	...
	},
	{
		...
		title: 'SEL 有效做法：短而明確，並放進班級日常',
		short_summary: '這組研究真正有用的訊息不是「SEL 有效果」四個字，而是哪種做法比較可能有效。2026 年《Review of Educational Research》統整 66 項研究、52,914 名 K-12 學生，發現 SEL 對親社會行為有小到中等的穩定效果；2025 年另一篇回顧納入 40 項研究、33,737 名學生，顯示學業成就也有小幅提升。換成白話：這不是立刻讓全班成績大跳升的方案，而是讓學生平均更願意合作、幫助、傾聽與留在學習任務裡。較早的大型 meta-analysis 則指出，成效較穩的 SEL 通常符合 SAFE 原則：有順序、要練習、聚焦少數技能、明確教出來。',
		emoji: '🤝',
		did_you_know: '效果量 g=.24 可粗略理解成：原本在平均位置的學生，介入後約移到第 59 百分位；g=.101 約移到第 54 百分位。這不是巨大效果，但若是全校、長期、低成本地嵌入日常，累積起來就值得看。',
		talking_points: [
			'怎麼看數字：g=.24、g=.101 都不是「強效藥」等級。比較合理的解讀是：SEL 若做得好，能讓班級互動與學業表現往好的方向小幅但穩定地移動。',
			'比較有效的做法：2011 年 213 項校本 SEL 研究指出，符合 SAFE 原則的方案較穩。S 是有順序的步驟，A 是讓學生主動練習，F 是聚焦少數技能，E 是把技能明確教出來。',
			'誰來做也重要：大型回顧發現，一般教師與學校人員在正常上課日執行 SEL 可以有效；學業成就提升主要出現在由學校人員執行的方案，而不是只靠外部講師。',
			'可以怎麼落地：每週固定 10 到 15 分鐘，練一個具體技能，例如求助句型、同儕回饋、衝突修復、情緒命名、分組角色；重點是反覆練習與回到課堂任務，不是一次性講座。',
			'統計上的提醒：2026 年親社會行為回顧發現，低於 9 小時的較短 SEL 課程反而效果較大，代表「短、清楚、可練習」可能比冗長課程更適合先試作。',
			'不要誤讀：SEL 不是把教師變諮商師，也不是萬用解方。若只是發學習單、喊口號、做一次活動，或沒有追蹤執行品質，研究不支持它一定有效。',
		],
		quote: null,
		quote_attribution: null,
		quote_source_url: null,
	...
	},
	{
		...
		category: '教育研究',
		editorial_lens: '自然探究',
		title: '自然探究怎麼教清楚：問題、變因、證據、論證',
		short_summary: '自然科學探究不是把學生丟去做活動就好。研究與科學教育框架都指向同一件事：探究要有效，教師必須把「問什麼、改變什麼、量什麼、用什麼證據說服別人」教成可操作流程。2012 年《Review of Educational Research》統整 37 項實驗與準實驗研究，平均效果量約 .50，且教師主導或引導的探究比完全學生主導更穩；2016 年另一篇 meta-analysis 納入 72 項研究，也顯示引導能提升學習活動、任務成功與學習結果。延伸到 108 課綱自然科學「探究與實作」，可把一節探究課拆成四個清楚教點：問題要能被資料回答，變因要能公平比較，證據要是可量測資料，論證要說出資料為什麼支持主張。',
		emoji: '🔬',
		did_you_know: '「控制變因」不是口號。Chen 與 Klahr 的研究把它稱為 CVS：一次只改變一個關鍵變因，其他條件維持一致，才有辦法做出有效因果推論。',
		talking_points: [
			...
			'問題怎麼教清楚：把題目改成「可用資料回答」的形式。例如不要只問「哪個比較好」，而是問「在相同光照時間下，不同肥料會如何影響豆苗高度？」',
			'變因怎麼教清楚：先讓學生列出可能影響結果的條件，再分成三欄：我要改變的變因、我要測量的結果、我要保持一樣的條件。這是控制變因策略，不是實驗單上的填空。',
			'證據怎麼教清楚：證據不是「我看到」或「我覺得」，而是能支持主張的特定資料。教師要要求學生指出哪一格數據、哪個趨勢、哪次觀察支持他的答案。',
			'論證怎麼教清楚：用 CER 做基本鷹架：主張是答案，證據是資料，推理是把資料連到科學概念的句子。最容易缺的是推理，學生常只貼數據，沒有說明為什麼數據能支持主張。',
			'對語言負荷高的學生：AERA Open 的回顧顯示，英語學習者接受探究式科學教學比直接教學有較佳科學成就，但探究本身很吃語言，所以句型、詞彙、圖表與討論規則都要明示。',
			'可試做一步：同一個實驗活動先補四張小卡：問題卡、變因卡、證據卡、CER 論證卡。每張卡只檢查一件事，避免學生被「完整報告」一次壓垮。',
