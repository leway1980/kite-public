...
const aiEduStories: Story[] = [
	{
		...
		talking_points: [
			'機構導入：喬治亞理工把 Gemini 與 NotebookLM 納入 OIT 管理的 enterprise service，優先向教職員開放，而不是讓所有使用者自行註冊。',
			'治理門檻：校方同步規劃首份 AI 政策，並把第三方安全評估、資料隱私、合規審查與 USG 規範一致性列為導入條件。',
			'產品訊號：KOC 與 SOGI 報導的是 NotebookLM 功能進入 Gemini、免費與付費版本並存；這代表可及性提高，但不是教學成效證據。',
			'教室試作：育才獨中的工作坊把 TalkPal、NotebookLM、ClassPoint 串成活動流程，重點是教師開始組合工具，而非單一工具取代教學。',
			'高中落地：先選非敏感教材做 2–4 週試行，設定不上傳個資、輸出需查核、學生帳號分層與成效紀錄，再評估是否擴大使用。',
		],
